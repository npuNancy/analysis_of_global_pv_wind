...
# -*- coding: utf-8 -*-
"""
RQ2 图 2：容量加权极端天气暴露时间序列。

本图把 Pipeline B 输出的场站级二值极端事件信号汇总为 26 个
regional-BCSD 国家年度全球暴露，并使用场站装机容量作为权重：

    exposed days per GW-year = sum(any_event_signal * capacity_gw) * dt_hours / 24 / active_capacity_gw
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RQ2：绘制容量加权全球极端暴露时间序列。",
...
def annual_weighted_file(path: Path, source: str, tech: str, target_years: list[int], dt_hours_arg: float | None):
    rows = []
    with h5py.File(path, "r") as f:
        ...
        for year in target_years:
            ...
            if denom <= 0 or event_any is None or not year_mask.any():
                ...
                signal_gw_days = float(event_any[year_mask].astype(float).dot(active_capacity).sum()) * dt_hours / 24.0
...
def build_summary(args: argparse.Namespace) -> tuple[pd.DataFrame, pd.DataFrame]:
            ...
            for ssp in args.ssps:
                for tech in ("wind", "solar"):
                    ...
                    if path is None:
                        ...
                        continue
                    file_rows = annual_weighted_file(path, source, tech, years, args.dt_hours)
    ...
    summary["exposed_days_per_gw_year"] = np.where(
        summary["active_capacity_gw"] > 0,
        summary["signal_gw_days"] / summary["active_capacity_gw"],
        0.0,
    )
...
def plot_summary(summary: pd.DataFrame, args: argparse.Namespace, out_dir: Path) -> Path:
    ...
    for ax, tech, tag in zip(axes, ["wind", "solar"], ["a", "b"]):
        ...
        for ssp in args.ssps:
            ...
            ax.plot(sub["year"], sub["exposed_days_per_gw_year"], "-", color=color, lw=1.8, label=label)
            add_linear_trend(ax, sub["year"], sub["exposed_days_per_gw_year"], color)
        ax.set_title(f"{TECH_LABEL[tech]}：容量加权极端暴露", fontsize=9)
        ax.set_xlabel("年份")
        ax.grid(axis="y", lw=0.4, alpha=0.45)
        panel_tag(ax, tag)

    axes[0].set_ylabel("极端暴露（天/GW年）")
    handles = [
    ...
    fig.suptitle(f"全球26国极端天气暴露时间序列（容量加权，{args.model}）", fontsize=11, fontweight="bold", y=1.02)
    fig.text(
        0.5,
        0.04,
        "指标为任一支持事件发生的3小时步数按容量加权后折算天数，并除以当年已投产容量；缺失国家-情景-技术文件按零贡献保留。",
        ha="center",
        fontsize=6.2,
        color="0.4",
    )

    out_base = out_dir / "fig_RQ2_extreme_exposure_timeseries_capacity_weighted"
...
def main() -> None:
    ...
    summary.to_csv(out_dir / "csv/RQ2_extreme_exposure_timeseries_capacity_weighted.csv", index=False)
    if not missing.empty:
        missing.to_csv(out_dir / "csv/RQ2_missing_capacity_weighted_inputs.csv", index=False)

    path = plot_summary(summary, args, out_dir)
    print(f"已保存：{path}")
    print(f"已保存汇总表：{out_dir / 'csv/RQ2_extreme_exposure_timeseries_capacity_weighted.csv'}")
    if not missing.empty:
        print(f"缺失组合记录：{out_dir / 'csv/RQ2_missing_capacity_weighted_inputs.csv'}")
