...
REGION_SHORT_CN = {
    1: "北美", 2: "中美", 3: "加勒比", 4: "南美",
    5: "北欧", 6: "西欧", 7: "南欧", 8: "东欧",
    9: "中亚", 10: "东亚", 11: "西亚", 12: "南亚",
    13: "东南亚", 14: "美拉", 15: "澳新",
    16: "北非", 17: "西非", 18: "中非", 19: "东非", 20: "南非",
}
...
def report_global(cov, out_path):
    ...
    print(f"  {'tech':<8}{'n_tot':>10}{'n_in':>10}{'n_cov':>8}   "
...
def report_by_macro_region(cov, region, transform, names, out_path):
    ...
    n_tot = np.zeros(n_ids + 1); n_in = np.zeros(n_ids + 1)
    c_tot = np.zeros(n_ids + 1); c_in = np.zeros(n_ids + 1)

    for tech in TECHS:
        d = cov[tech]
        rid = assign_region(d["lon"], d["lat"], region, transform)
        cap = d["cap"]; covered = d["covered"]
        valid = rid > 0
    ...
    with open(out_path, "w", newline="") as f:
        wr = csv.writer(f)
        wr.writerow(["region_id", "region_name",
        ...
        for rid in range(1, n_ids + 1):
            ...
            print(f"  {rid:<3}{name:<24}"
                  f"{nt:>9,}{ni:>9,}{nr:>6.1%}   "
                  f"{ct:>10,.1f}{ci:>10,.1f}{cr:>7.1%}")
            wr.writerow([rid, name, nt, ni, f"{nr:.4f}",
        ...
        print(f"  {'':3}{'20 区合计':<24}"
              f"{NT:>9,}{NI:>9,}{nr:>6.1%}   "
              f"{CT:>10,.1f}{CI:>10,.1f}{cr:>7.1%}")
        wr.writerow(["ALL", "20 区合计", NT, NI, f"{nr:.4f}",
                     f"{CT:.4f}", f"{CI:.4f}", f"{cr:.4f}"])
    print(f"  -> {out_path}")
    return region_rows
...
def plot_coverage_map(cov, domain, region_geoms, names, out_path):
    ...
    for ax, tech in zip(axes, TECHS):
        ...
        ax.scatter(lon[~m], lat[~m], s=1.2, c="#b0b0b0", transform=PC,
                   label=f"未覆盖 ({int((~m).sum()):,})", rasterized=True, zorder=3)
        ax.scatter(lon[m], lat[m], s=1.2, c="#d62728", transform=PC,
                   label=f"被项目区域覆盖 ({int(m.sum()):,})", rasterized=True, zorder=4)
        ...
        ax.set_title(f"GRW {TECH_LABEL[tech]}场站覆盖："
                     f"场站 {int(m.sum()/len(lon)*100):d}% / 装机 {cr:.0%}",
                     fontsize=14, pad=10)
    plt.tight_layout()
...
def plot_macro_bar(region_rows, out_path, threshold=60.0):
    ...
    ax.bar(x, cap_ratio, width=0.72, color=colors,
           edgecolor="#666666", linewidth=0.4, zorder=3)

    for xi, r in zip(x, cap_ratio):
        ax.text(xi, r + 1.2, f"{r:.0f}%", ha="center", va="bottom",
                fontsize=6.5, color="#222222", zorder=4)

    # 阈值红线（默认 60%）
    ax.axhline(threshold, color="#d62728", lw=1.3, ls="--", zorder=5)
    ax.text(n - 0.45, threshold + 0.8, f"{threshold:.0f}% 阈值", color="#d62728",
            ha="right", va="bottom", fontsize=7.5, zorder=6)
    ...
    ax.set_title("全球 20 大区：GRW 现状装机容量被项目区域覆盖的比例"
                 "（x 轴下方为区内总装机 GW）", fontsize=10.5, pad=8)
...
def main():
    ...
    ap.add_argument("--draw-macro-regions", action="store_true",
                    help="分布图叠加 20 大区边界（默认不画；大区统计/柱状图始终输出）")
    args = ap.parse_args()
    ...
    if region is not None:
        region_rows = report_by_macro_region(
            cov, region, transform, names,
            os.path.join(OUTPUT_DIR, "grw_coverage_by_macro_region.csv"))
        plot_macro_bar(region_rows,
                       os.path.join(OUTPUT_DIR, "grw_coverage_macro_bar.png"),
                       threshold=args.threshold)

    if not args.no_map:
        # 默认不画 20 大区边界；仅 --draw-macro-regions 时才矢量化（较耗时）
        region_geoms = (macro_region_geometries(grid, transform)
                        if (args.draw_macro_regions and grid is not None) else {})
        plot_coverage_map(cov, domain, region_geoms, names,
                          os.path.join(OUTPUT_DIR, "grw_coverage_map.png"))
