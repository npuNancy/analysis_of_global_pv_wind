#!/usr/bin/env python3
"""经度带（全球 6×60° 扇区）相对损失率分析的数据入口与绘图工具。"""
...
def aggregate_band_rates(data: pd.DataFrame) -> pd.DataFrame:
    """按（经度带，情景，技术，快照）先汇总分子分母，再计算相对损失率。"""
    ...
    valid = (
        np.isfinite(grouped["net_loss_mwh"]) & np.isfinite(grouped["normal_all_mwh"])
        & grouped["normal_all_mwh"].gt(0)
    )
    grouped["relative_loss_rate"] = np.where(
        valid, grouped["net_loss_mwh"] / grouped["normal_all_mwh"] * 100, np.nan
    )
            ...
            for scenario in COMPARE_SSPS:
                for year in SNAPSHOTS:
                    ...
                    rows.append(dict(
                        ...
                        relative_loss_rate=np.nan if row is None else float(row["relative_loss_rate"]),
