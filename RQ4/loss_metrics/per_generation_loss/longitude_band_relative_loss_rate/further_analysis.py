#!/usr/bin/env python3
"""基于经度带相对损失率的进一步分析：带间分异、SSP 敏感度、趋势、构成与混杂因素。"""
...
def band_disparity(metrics: pd.DataFrame) -> pd.DataFrame:
    """每（技术，情景，年代）内最高/最低带、极差（pp）及归一化离散度。"""
    rows = []
    for keys, group in metrics.groupby(["tech", "scenario", "snapshot_year"]):
        valid = group[group["relative_loss_rate"].notna()]
        if len(valid) < 2:
            continue
        top = valid.loc[valid["relative_loss_rate"].idxmax()]
        bottom = valid.loc[valid["relative_loss_rate"].idxmin()]
        mean = valid["relative_loss_rate"].mean()
        rows.append(dict(
            tech=keys[0], scenario=keys[1], snapshot_year=keys[2],
            max_band=top["lon_band"], max_pct=top["relative_loss_rate"],
            min_band=bottom["lon_band"], min_pct=bottom["relative_loss_rate"],
            range_pp=top["relative_loss_rate"] - bottom["relative_loss_rate"],
            cv_pct=valid["relative_loss_rate"].std(ddof=1) / mean * 100,
            n_bands=len(valid),
        ))
...
def plot_disparity(disparity: pd.DataFrame, output_dir: Path) -> Path:
    ...
    for ax, tech in zip(axes, TECHS):
        ...
        for scenario in COMPARE_SSPS:
            ...
            ax.plot(s["snapshot_year"], s["range_pp"], "-o", color=SSP_COLOR[scenario],
        ...
        if ax is axes[0]:
            ax.set_ylabel("Max–min band range (pp)")
    fig.suptitle("Longitude-band disparity of relative loss rate (max − min)",
