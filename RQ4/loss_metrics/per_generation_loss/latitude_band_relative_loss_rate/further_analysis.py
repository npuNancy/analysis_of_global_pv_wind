#!/usr/bin/env python3
"""基于纬度带相对损失率的进一步分析：梯度、不对称、SSP 敏感度、趋势、构成与混杂因素。"""
...
def load_full(station_csv: Path, model: str) -> pd.DataFrame:
    ...
    # 与 common.load_source 相同的临时风电修正（对相对率与占比无影响，仅统一绝对能量列）。
...
def latitude_gradient(metrics: pd.DataFrame) -> pd.DataFrame:
    """以带中心纬度（N:+65/+40/+15，S:-15/-40/-65）回归相对损失率，输出斜率（pp/°）。"""
    ...
    for keys, group in metrics.groupby(["tech", "scenario", "snapshot_year"]):
        valid = group[group["relative_loss_rate"].notna()]
        if len(valid) < 3:
            continue
        x = valid["lat_band"].map(center).to_numpy(dtype=float)
        y = valid["relative_loss_rate"].to_numpy(dtype=float)
        slope, intercept = np.polyfit(x, y, 1)
        r = float(np.corrcoef(x, y)[0, 1])
        rows.append(dict(
            tech=keys[0], scenario=keys[1], snapshot_year=keys[2],
            slope_pp_per_deg=float(slope), intercept_pp=float(intercept),
        ...
        if north in pivot.columns and south in pivot.columns:
            ...
            for (tech, scenario, snapshot), value in diff.dropna().items():
                rows.append(dict(
                    ...
                    band=band, north_pct=pivot.loc[(tech, scenario, snapshot), north],
                    south_pct=pivot.loc[(tech, scenario, snapshot), south],
                    north_minus_south_pp=float(value),
                ))
    return pd.DataFrame(rows)
...
def ssp_sensitivity(metrics: pd.DataFrame) -> pd.DataFrame:
    """每带相对损失率的情景跨度（ssp585 - ssp126）及其占 ssp126 水平的比例。"""
    pivot = metrics.pivot_table(
        index=["tech", "lat_band", "snapshot_year"], columns="scenario",
        values="relative_loss_rate",
    )
    pivot["ssp585_minus_126_pp"] = pivot["ssp585"] - pivot["ssp126"]
    pivot["ssp585_minus_245_pp"] = pivot["ssp585"] - pivot["ssp245"]
    pivot["span_ratio_vs_126"] = pivot["ssp585_minus_126_pp"] / pivot["ssp126"] * 100
...
def time_trend(metrics: pd.DataFrame) -> pd.DataFrame:
    """带内 2030s→2050s 相对损失率变化（pp），按情景。"""
    pivot = metrics.pivot_table(
        index=["tech", "lat_band", "scenario"], columns="snapshot_year",
        values="relative_loss_rate",
    )
    pivot["change_2030s_to_2050s_pp"] = pivot[2050] - pivot[2030]
    pivot["relative_change_pct"] = pivot["change_2030s_to_2050s_pp"] / pivot[2030] * 100
    return pivot.reset_index()
...
def global_contribution(metrics: pd.DataFrame) -> pd.DataFrame:
    """每带净损失占全部带总和的比例（能量加权，按情景），及其装机占比对照。"""
    ...
    merged["rate_ratio_vs_global"] = (
        merged["relative_loss_rate"]
        / (merged["total_net_loss_mwh"] / merged["total_normal_all_mwh"] * 100)
    )
    return merged
...
def plot_gradient_heatmap(gradients: pd.DataFrame, output_dir: Path) -> Path:
    """斜率（pp/°）随情景与年代的演变：纬度梯度是增强还是减弱。"""
    configure_style()
    fig, axes = plt.subplots(1, 2, figsize=(7.2, 2.8), sharey=True)
    for ax, tech in zip(axes, TECHS):
        sub = gradients[gradients["tech"].eq(tech)].pivot(
            index="scenario", columns="snapshot_year", values="slope_pp_per_deg",
        ...
        fig.colorbar(im, ax=ax, fraction=0.04, pad=0.03, label="pp per ° latitude")
    fig.suptitle("Latitude gradient of relative loss rate (linear fit, 6 bands)",
...
def plot_asymmetry(asym: pd.DataFrame, output_dir: Path) -> Path:
    ...
    for ax, tech in zip(axes, TECHS):
            ...
            for i, band in enumerate(bands):
                for year in SNAPSHOTS:
                    ...
                    if len(match):
                        ...
                        ys.append(float(match["north_minus_south_pp"].iloc[0]))
        ...
        if ax is axes[0]:
            ax.set_ylabel("N minus S (pp)")
    fig.suptitle("Hemispheric asymmetry of relative loss rate by band (bar = decade)",
                 fontsize=9, fontweight="bold")
    fig.tight_layout()
...
def main() -> None:
    ...
    metrics["relative_loss_rate"] = np.where(
        metrics["normal_all_mwh"].gt(0),
        metrics["net_loss_mwh"] / metrics["normal_all_mwh"] * 100, np.nan,
    )
