...
METRICS = (MetricSpec("relative_loss_rate", "relative loss rate", "%"),)
...
def aggregate_rates(data: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    valid = (
        np.isfinite(annual["net_loss_mwh"]) & np.isfinite(annual["normal_all_mwh"])
        & annual["normal_all_mwh"].gt(0)
    )
    annual["relative_loss_rate"] = np.where(
        valid, annual["net_loss_mwh"] / annual["normal_all_mwh"] * 100, np.nan
    )
    rows = []
    for keys, group in annual.groupby(["region", "scenario", "tech", "snapshot_year"]):
        ...
        if not np.isfinite(group[["net_loss_mwh", "normal_all_mwh"]].to_numpy()).all():
            reasons.append("nonfinite_energy")
        if not group["normal_all_mwh"].gt(0).all():
            reasons.append("nonpositive_normal_generation")
        loss = group["net_loss_mwh"].sum()
        normal = group["normal_all_mwh"].sum()
        rows.append(dict(
            region=region, scenario=scenario, tech=tech, snapshot_year=snapshot,
            metric="relative_loss_rate", value=loss / normal * 100 if not reasons else np.nan,
            n_years=group["analysis_year"].nunique(), net_loss_mwh=loss,
            normal_all_mwh=normal, reason=";".join(reasons) or "valid",
        ))
