#!/usr/bin/env python3
"""按南北半球×低中高纬度带对比单位装机损失轨迹，并分解带内极端事件。"""
...
def plot_band_trajectories(
    ...
    *,
    ...
    """三个 SSP 面板内画各纬度带的单位装机损失轨迹。"""
    configure_style()
    subset = metrics[metrics["tech"].eq(tech)]
    bands = [band for band in LAT_BANDS if (subset["lat_band"] == band).any()]
    ...
    for col, scenario in enumerate(COMPARE_SSPS):
        ...
        for band in bands:
            group = subset[
                subset["lat_band"].eq(band) & subset["scenario"].eq(scenario)
            ].set_index("snapshot_year").reindex(SNAPSHOTS)
            ax.plot(
                SNAPSHOTS, group["unit_capacity_loss"],
        ...
        if col == 0:
            ax.set_ylabel("Unit-capacity loss (MWh MW$^{-1}$ yr$^{-1}$)")
    ...
    fig.suptitle(f"{TECH_LABEL[tech]}: unit-capacity-loss trajectories by latitude band",
                 fontsize=10, y=1.12)
...
def plot_band_events(
    ...
    *,
    ...
    bands = [band for band in LAT_BANDS if (data["lat_band"] == band).any()]
    ...
    for ax, event_order, column, title in [
        ...
        for band_index, band in enumerate(bands):
            ...
            ax.text(center, -0.18, BAND_LABEL[band].split(" (")[0], transform=ax.get_xaxis_transform(),
    ...
    fig.suptitle(
        f"{TECH_LABEL[tech]}: event-loss composition by latitude band ({snapshot}s)",
        fontsize=9.5, fontweight="bold", y=1.04,
    )
    fig.subplots_adjust(left=0.09, right=0.985, bottom=0.19, top=0.80, wspace=0.28)
    stem = f"fig_RQ4_band_event_composition_{tech}_{snapshot}"
...
def main() -> None:
    ...
    config["band_edges"] = list(BAND_EDGES)
    config["lat_bands"] = list(LAT_BANDS)
    config["aggregation"] = "sum(decadal net loss) / sum(capacity) / 10 years, capacity-weighted within band"
    (output_dir / "run_config.json").write_text(json.dumps(config, indent=2), encoding="utf-8")

    coverage.to_csv(csv_dir / "band_coverage.csv", index=False)
    metrics.to_csv(csv_dir / "RQ4_band_metric_decade.csv", index=False)
    events.to_csv(csv_dir / "RQ4_band_event_decade.csv", index=False)
    ...
    for tech in args.techs:
        ...
        n_bands = tech_metrics["lat_band"].nunique()
        print(f"完成 {tech}：{n_bands} 个纬度带。")
