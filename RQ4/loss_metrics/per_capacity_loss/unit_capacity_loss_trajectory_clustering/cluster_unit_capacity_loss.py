#!/usr/bin/env python3
"""仅按三种 SSP 下单位装机损失轨迹聚类国家，并分解类内极端事件。"""
...
def build_feature_matrix(
    ...
    *,
    ...
    """构造九维单位装机损失轨迹，并按列进行跨国家 z-score 标准化。"""
...
def plot_clusters(assignments, scores, tree, *, tech, metric, output_dir) -> Path:
    ...
    fig.suptitle(f"{TECH_LABEL[tech]}: unit-capacity-loss trajectories by cluster", fontsize=10, y=1.10)
...
def main() -> None:
    ...
    config["aggregation"] = "annual net loss / capacity; decade metric = sum(net_loss) / capacity / 10 years"
    ...
    for tech in args.techs:
        for metric in args.metrics:
            ...
            pd.DataFrame({"feature": FEATURE_NAMES, "mean_value": raw_features.mean(axis=0),
                          "std_value": raw_features.std(axis=0, ddof=1)}).to_csv(
