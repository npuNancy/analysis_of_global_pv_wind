...
# -*- coding: utf-8 -*-
"""
RQ1：未来气候如何改变风/光的场站发电量（出力）—— 与 CF 图四子图保持一致的版本。

四个子图与 plot_RQ1_cf.py 完全对应（仅把指标由容量因子换成发电量）：
  (a) 全球发电量时间轨迹（按 SSP）
  (b) 场站级发电量分布：嵌套小提琴（2030/2040/2050，宽度∝场站数，对数轴）
  (c) 各国发电量变化热图（2050 vs 2030，%）
  (d) 变化幅度最大的国家（SSP5-8.5）

框架与 CF 图一致：固定部署机队（deploy_ssp = ssp245），仅变化 climate_ssp。
注意：发电量随时间增长主要来自装机扩张，气候是次要项（表现为 SSP 间的小幅差异）。
（mock 数据无气候模型维度，按要求当作 NESM3 情形。）

输出：outputs/RQ1_future_generation/fig_GENcf_{solar,wind}.png
"""
import os
import numpy as np
import pandas as pd
...
# --------------------------------------------------------------------------- #
# 辅助函数（与 CF 脚本一一对应，把 CF 换成发电量）
# --------------------------------------------------------------------------- #
def global_gen(tech, deploy="ssp245"):
    """固定机队下，按（climate_ssp, 年份）汇总全球年发电量（TWh）。"""
    d = country[(country.technology == tech) & (country.deploy_ssp == deploy)]
    g = d.groupby(["climate_ssp", "target_year"]).agg(
        gen=("annual_generation_mwh", "sum")).reset_index()
    g["gen_twh"] = g.gen / 1e6                                  # MWh → TWh
    return g


def country_gen_change(tech, deploy="ssp245"):
    """各国 × climate_ssp 的发电量变化率（2050 相对 2030，%）。"""
    d = country[(country.technology == tech) & (country.deploy_ssp == deploy)]
...
def gen_array(tech, climate, year, deploy="ssp245"):
    """某技术 / 气候情景 / 年份下，全部场站的年发电量（GWh）。"""
    d = st_ann[(st_ann.technology == tech) & (st_ann.deploy_ssp == deploy)
               & (st_ann.climate_ssp == climate) & (st_ann.target_year == year)]
    return d.annual_generation_mwh.values / 1e3                 # MWh → GWh


def gen_log_limits(tech):
    """该技术全部 (情景×年份) 发电量的 log10 范围，用于统一对数网格与坐标。"""
    vals = np.concatenate([gen_array(tech, s, y) for s in SSPS for y in YEARS])
...
# =========================================================================== #
# 发电量图（风/光各一张）—— 四子图与 CF 图保持一致
# =========================================================================== #
def figure_gen(tech):
    g = global_gen(tech)
    cc = country_gen_change(tech)           # 各国 × ssp（%）
    ...
    # (b) 场站级发电量分布：嵌套小提琴（2030/2040/2050，对数轴） -------------
    # 同一 SSP 位置叠放 3 个年份，宽度按场站数缩放；场站数逐年增加，
    ...
    for j, s in enumerate(SSPS):
        arrs = {y: gen_array(tech, s, y) for y in YEARS}
        # 在对数空间做核密度估计，再乘场站数得到计数密度
        ...
        for y in [2050, 2040, 2030]:               # 由宽到窄、从后往前画
            d = dens[y] * scale
            axb.fill_betweenx(ygrid, j - d, j + d, color=SSP_C[s],
    ...
    axb.set_ylabel("场站年发电量 (GWh，对数)")
    axb.set_ylim(10 ** loglo, 10 ** loghi)
    axb.set_title("场站发电量分布（嵌套：宽度∝场站数）", fontsize=8.5)
    ...
    axc.set_title("各国发电量变化（2050 vs 2030，%）", fontsize=8.5)
    cb = fig.colorbar(im, ax=axc, fraction=0.046, pad=0.03)
    cb.set_label("变化率 (%)", fontsize=7)
    cb.ax.tick_params(labelsize=6)
    panel_tag(axc, "c")

    # (d) SSP5-8.5 下变化幅度最大的国家排序 ----------------------------------
    axd = fig.add_subplot(gs[1, 1])
    r = cc["ssp585"].sort_values()
    sel = pd.concat([r.head(8), r.tail(8)])                    # 最低与最高各取 8
    ...
    axd.set_xlabel("发电量变化 2050 vs 2030 (%)")
    axd.set_title("变化幅度最大的国家（SSP5-8.5）", fontsize=8.5)
    panel_tag(axd, "d")
    ...
    fig.text(0.5, 0.005,
             "固定部署机队（deploy = SSP2-4.5），仅变化气候情景；28 个区域。"
             "随时间的增长主要来自装机扩张，气候为次要项（SSP 间的差异）。",
             ha="center", fontsize=6.2, color="0.4")
    p = f"{OUT}/fig_GENcf_{tech}.png"
