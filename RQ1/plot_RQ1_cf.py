...
# --------------------------------------------------------------------------- #
def global_cf(tech):
    """全球各站 CF 算术平均（排除零值站，自洽情景）。"""
    d = st_ann[(st_ann.technology == tech) & (st_ann.annual_capacity_factor > 0)]
    g = (d.groupby(["climate_ssp", "target_year"])["annual_capacity_factor"]
          .mean().reset_index()
          .rename(columns={"annual_capacity_factor": "cf"}))
    return g


def country_cf_change(tech):
    """各国 × climate_ssp 的 CF 变化率（2050 vs 2030，%）。
    缺失数据（国家/SSP 组合不存在）以 NaN 填充。"""
    d = country[country.technology == tech]
    p = d.pivot_table(index=["country", "climate_ssp"], columns="target_year",
                      values="mean_cf")
    # 部分国家可能缺 2030 数据（全部站 activation_year > 2030）
...
def figure_cf(tech):
    ...
    for s in SSPS:
        ...
        if sub.empty:
            continue
        axa.plot(sub.target_year, sub.cf * 100, "-o", color=SSP_C[s],
    ...
    # (c) 各国 CF 变化热图（NaN 以灰色显示）--------------------------------------
    axc = fig.add_subplot(gs[1, 0])
    if cc.empty:
        axc.text(0.5, 0.5, "数据不足", transform=axc.transAxes,
                 ha="center", va="center")
    else:
        # 只保留 SSPS 中实际存在的列
        cols_avail = [s for s in SSPS if s in cc.columns]
        order = cc[cols_avail].mean(axis=1).sort_values().index
        M = cc.reindex(columns=SSPS).loc[order].values.astype(float)
        vmax = np.nanmax(np.abs(M))
        if vmax == 0 or np.isnan(vmax):
            vmax = 1
        norm = TwoSlopeNorm(vmin=-vmax, vcenter=0, vmax=vmax)
        cmap_hm = plt.get_cmap(DIVMAP).copy()
        cmap_hm.set_bad(color="0.85")
        im = axc.imshow(np.ma.masked_invalid(M), aspect="auto",
                        cmap=cmap_hm, norm=norm)
    ...
    # (d) SSP5-8.5 下涨跌幅最大的国家 -------------------------------------------
    axd = fig.add_subplot(gs[1, 1])
    if not cc.empty and "ssp585" in cc.columns:
        r = cc["ssp585"].dropna().sort_values()
        n_sel = min(8, len(r) // 2)
        sel = pd.concat([r.head(n_sel), r.tail(n_sel)])
        colors = [SSP_C["ssp585"] if v >= 0 else SSP_C["ssp126"] for v in sel.values]
        axd.barh(range(len(sel)), sel.values, color=colors,
                 edgecolor="white", lw=0.4)
        axd.axvline(0, color="0.3", lw=0.7)
        axd.set_yticks(range(len(sel)))
        axd.set_yticklabels(sel.index, fontsize=6)
        axd.set_xlabel("CF 变化 2050 vs 2030 (%)")
        axd.set_title("涨跌幅最大的国家（SSP5-8.5）", fontsize=8.5)
    else:
        axd.text(0.5, 0.5, "SSP5-8.5 数据不足", transform=axd.transAxes,
                 ha="center", va="center")
    panel_tag(axd, "d")

    tech_cn = {"solar": "光伏", "wind": "风电"}[tech]
    fig.suptitle(f"未来气候对{tech_cn}容量因子的影响（NESM3）",
                 fontsize=11, fontweight="bold", y=0.975)
    fig.text(0.5, 0.005,
             "自洽情景（部署=气候）；NESM3 模型；CF 为各站算术平均（排除零值站）；灰色格子=数据缺失。",
