...
def monthly_cycle(tech, climate, year):
    ...
    g = d.groupby("month").apply(lambda x: x.wcf.sum() / x.capacity_mw.sum(), include_groups=False)
...
def country_segments(tech, ntop=8):
    ...
    for s in SSPS:
        ...
        if sub.empty:
            ...
            p = (
                sub.pivot_table(index="country", columns="target_year", values="annual_generation_mwh").reindex(order)
                / 1e6
            )
            for yr in YEARS:
...
def figure_gen(tech):
    ...
    gs = fig.add_gridspec(
        3, 2, height_ratios=[0.95, 1.65, 0.85], hspace=0.5, wspace=0.3, left=0.1, right=0.96, top=0.92, bottom=0.07
    )
    ...
    for s in SSPS:
        ...
        if sub.empty:
            continue
        axa.plot(sub.target_year, sub.gen_twh, "-o", color=SSP_C[s], lw=1.8, ms=4, label=SSP_L[s])
    ...
    if countries:
        ...
        for ci, c_ in enumerate(countries):
            for si, s in enumerate(SSPS):
                ...
                axc.barh(y, r.inc40, height=bar_h, left=r.base, color=cols[1], edgecolor="white", lw=0.4)
                axc.barh(y, r.inc50, height=bar_h, left=r.base + r.inc40, color=cols[2], edgecolor="white", lw=0.4)
    ...
    if countries:
        for ci, c_ in enumerate(countries):
            ...
            for si, s in enumerate(SSPS):
                ...
                axd.barh(y, i50, height=bar_h, left=b + i40, color=cols[2], edgecolor="white", lw=0.4)
                axd.text(b + i40 + i50, y, f" {r.total:.0f}", va="center", ha="left", fontsize=4.6, color="0.2")
    ...
    for j, s in enumerate(SSPS):
        ...
        if any(len(v) < 3 for v in arrs.values()):
            continue
        dens = {y: gaussian_kde(np.log10(arrs[y]))(grid_log) * len(arrs[y]) for y in YEARS}
    ...
    handles_vio.append(Line2D([], [], marker="o", ls="", mfc="white", mec="k", mew=0.8, ms=4, label="中位数"))
    ...
    axl.set_xlim(0, 1)
    axl.set_ylim(0, 1)
    axl.text(0.04, 0.95, "子图 c / d 图例", fontsize=7.5, fontweight="bold", va="top")
    x0, y_top = 0.42, 0.74
    cw, ch = 0.16, 0.17
    for j, lab in enumerate(SEG_LABELS):
        axl.text(x0 + (j + 0.5) * cw, y_top + 0.04, lab, ha="center", va="bottom", fontsize=5.6, rotation=22)
    for i, s in enumerate(SSPS):
        yc = y_top - (i + 1) * ch
        axl.text(x0 - 0.03, yc + ch * 0.5, SSP_L[s], ha="right", va="center", fontsize=6.8)
        for j in range(3):
            axl.add_patch(Rectangle((x0 + j * cw, yc), cw, ch, facecolor=SSP_SHADES[s][j], edgecolor="white", lw=0.6))
