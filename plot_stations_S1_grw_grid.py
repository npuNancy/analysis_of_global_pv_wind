...
from plot_stations_S1_with_regions import PC, setup_basemap, in_any_region, load_nam_domain
...
def report_global(grids_by_res, stations, domain, out_path):
    ...
    print(f"\n  ── 全球格占用率 / 格数覆盖率（各分辨率）──")
    header = ["res", "tech", "n_cells_total", "n_cells_occupied", "occ_ratio",
              "n_cells_covered", "cell_cover_ratio", "cap_total_gw", "cap_cover_ratio"]
    rows = []
    for res_key in ("1deg", "01deg"):
        ...
        for tech in TECHS:
            ...
            print(f"    {TECH_LABEL[tech]:<4}: 占用 {n_occ:>7,} 格({occ_r:5.1%})，"
                  f"含覆盖 {n_cov:>7,} 格({cell_cov_r:5.1%})，"
                  f"装机 {cap_tot:>9,.0f} GW(覆盖 {cap_cov_r:5.1%})")
            rows.append((rl, tech, n_total_land, n_occ, f"{occ_r:.4f}",
                         n_cov, f"{cell_cov_r:.4f}", f"{cap_tot:.4f}", f"{cap_cov_r:.4f}"))
        ...
        print(f"    {'合计':<4}: 占用 {n_occ_combined:>7,} 格({occ_r_combined:5.1%})，"
              f"含覆盖 {n_cov_combined:>7,} 格({cell_cov_r_combined:5.1%})，"
              f"装机 {cap_tot_combined:>9,.0f} GW(覆盖 {cap_cov_r_all:5.1%})")
        rows.append((rl, "all", n_total_land, n_occ_combined, f"{occ_r_combined:.4f}",
                     n_cov_combined, f"{cell_cov_r_combined:.4f}",
                     f"{cap_tot_combined:.4f}", f"{cap_cov_r_all:.4f}"))
...
def report_by_macro_region(macro_grid, grids, names, out_path, res_label):
    ...
    with open(out_path, "w", newline="") as f:
        ...
        wr.writerow([
            ...
            "n_land_cells", "n_occ_solar", "n_occ_wind", "n_occ_combined", "occ_ratio",
            "cap_solar_gw", "cap_wind_gw", "cap_total_gw",
        ])
        for rid in range(1, N_REGIONS + 1):
            ...
            wr.writerow([rid, name, nl, ns, nw, n_all, f"{occ_r:.4f}",
                         f"{cs:.4f}", f"{cw:.4f}", f"{cap_all:.4f}"])
        ...
        wr.writerow(["ALL", "合计", nl_tot, "", "", n_all_tot, f"{occ_r_tot:.4f}",
                     "", "", f"{cap_tot:.4f}"])
    print("  " + "-" * 78)
    print(
...
def plot_macro_bar(region_rows, out_path, res_label, threshold=30.0):
    ...
    ax.text(
        n - 0.45, threshold + 0.5, f"{threshold:.0f}% 阈值",
        color="#d62728", ha="right", va="bottom", fontsize=7.5, zorder=6,
    )
    ...
    ax.set_title(
        f"全球 20 大区 GRW 现状风光场站陆地格占用率（{res_label}）"
        "（x 轴下方为区内合计装机 GW）",
        fontsize=10.5, pad=8,
    )
    fig.tight_layout()
...
def plot_capacity_bar(region_rows, out_path, threshold=60.0):
    ...
    for xi, r in zip(x, cap_ratio):
        ax.text(xi, r + 1.2, f"{r:.0f}%", ha="center", va="bottom", fontsize=6.5,
                color="#222222", zorder=4)

    # 阈值红线（默认 60%，与 coverage 脚本一致）
    ax.axhline(threshold, color="#d62728", lw=1.3, ls="--", zorder=5)
    ax.text(n - 0.45, threshold + 0.8, f"{threshold:.0f}% 阈值",
            color="#d62728", ha="right", va="bottom", fontsize=7.5, zorder=6)
    ...
    for sp in ("top", "right"):
        ax.spines[sp].set_visible(False)
    ax.set_title(
        "全球 20 大区：GRW 现状装机容量被项目区域覆盖的比例"
        "（场站级，与网格粒度无关；x 轴下方为区内总装机 GW）",
        fontsize=10.5, pad=8,
    )
    fig.tight_layout()
    fig.savefig(out_path, dpi=300, bbox_inches="tight")
...
def main():
    ...
    ap.add_argument(
        "--cap-threshold", type=float, default=60.0,
