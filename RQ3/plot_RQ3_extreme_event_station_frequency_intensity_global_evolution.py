...
图采用 2×2 布局：上排为事件频次，下排为单次事件损失；左列为风电，
右列为光伏。episode 及净损失沿用全球频次—条件强度图的汇总结果。
"""
...
OUT_STEM = "fig_RQ3_extreme_event_station_frequency_intensity_global_evolution"
TOTAL_OUT_STEM = "fig_RQ3_extreme_event_station_total_frequency_intensity_global_evolution"
CSV_NAME = "RQ3_extreme_event_station_frequency_intensity_global_evolution.csv"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RQ3：绘制场站级极端事件频次与单次事件净损失年代演化图。",
...
def load_summary(args: argparse.Namespace) -> pd.DataFrame:
    ...
    data["factorized_mean_loss_per_station_year_mwh"] = (
        data["event_frequency_per_station_year"] * data["loss_per_station_episode_mwh"]
    )
    data["factorization_abs_error_mwh"] = (
        data["mean_loss_per_station_year_mwh"]
        - data["factorized_mean_loss_per_station_year_mwh"]
    ).abs()
...
def plot_summary(
    ...
    *,
    ...
    for column, tech in enumerate(TECHS):
        ...
        plot_metric(
            ...
            value_column=(
                ...
                if total_count
                else "event_frequency_per_station_year"
            ),
        )
        plot_metric(
            axes[1, column],
            tech_data,
            args,
            value_column="loss_per_station_episode_mwh",
        )
    ...
    fig.suptitle(
        (
            f"极端事件总次数与单次事件净损失（场站级分解，{args.model}）"
            if total_count
            else f"场站级极端事件频次与单次事件净损失（{args.model}）"
        ),
        fontsize=11,
        fontweight="bold",
        y=1.0,
    ...
    fig.text(
        ...
        (
            ...
            if total_count
            else "场站平均年损失 = episode 数/(场站×年) × 净出力损失/episode；episode 为事件并集的 False→True 起点。"
        ),
        ha="center",
        fontsize=6.2,
        color="0.4",
...
def main() -> None:
    ...
    print(f"分解最大绝对误差：{summary['factorization_abs_error_mwh'].max():.3e} MWh/(场站·年)")
