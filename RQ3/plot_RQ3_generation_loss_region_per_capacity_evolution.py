...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="RQ3：逐国绘制风光单位装机净出力损失年代演化图。",
...
def load_region_loss_per_capacity_summary(args: argparse.Namespace) -> pd.DataFrame:
    """按国家逐年累计净损失和装机容量后相除，再做年代统计。"""
    ...
    apply_temp_wind_energy_and_capacity_correction(d, context="国家单位装机净出力损失图")
...
def plot_region(
    ...
    *,
    ...
    fig.suptitle(
        f"{region}：极端天气风光单位装机净出力损失演化（{args.model}）",
        fontsize=11,
        fontweight="bold",
        y=1.02,
    )
    ...
    out_path = out_dir / f"fig_RQ3_{safe_region_name(region)}_generation_loss_per_capacity_region_evolution{suffix}.png"
...
def main() -> None:
    ...
    summary = load_region_loss_per_capacity_summary(args)
    ...
    for region in regions:
        ...
        csv_path = (
            region_dir
            / f"RQ3_{safe_region}_generation_loss_per_capacity_region_evolution.csv"
        )
