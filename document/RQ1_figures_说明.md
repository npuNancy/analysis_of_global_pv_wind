...
> 关键方法：
> - **容量因子（CF）** 纯由气候驱动 → 采用自洽情景（`deploy_ssp == climate_ssp`），SSP 间的 CF 差异即气候信号。
> - **发电量** = 装机 × CF，同时受装机与气候影响 → 同样采用自洽情景（`deploy_ssp == climate_ssp`）画真实轨迹。
...
风电、光伏各输出一张 4 子图（a/b/c/d）。自洽情景（部署=气候），各 SSP 间的 CF 差异即气候信号。
...
风电、光伏各输出一张 **5 子图（a/b/c/d/e）**。全部采用自洽情景（部署=气候）。
...
| **b** | 季节循环位移：2030 vs 2050 | 月份（1–12） | 容量加权 CF (%) | 灰=2030 基准（SSP5-8.5）；蓝虚线=2050 SSP1-2.6；红=2050 SSP5-8.5；自洽情景（deploy=climate） |
| **c** | Top 8 国分情景 × 分时段发电量（绝对值） | 年发电量 (TWh) | 8 个主要国家 | 每国 3 柱（=3 个 SSP），柱内按时段堆叠（深=2030 基底、中=2030→2040 增量、浅=2040→2050 增量） |
| **d** | Top 8 国相对值（归一到 SSP1-2.6=100%） | 相对 SSP1-2.6 (%) | 同 c | 柱顶标注 2050 绝对发电量（TWh），竖虚线=100% |
| **e** | 场站发电量分布（嵌套小提琴，对数轴） | SSP 情景（3 类） | 场站年发电量 (GWh，对数) | 同一 SSP 叠放 2030/2040/2050；宽度∝场站数（2050 最宽）；白点=各年中位数 |

**可读出**：发电量增长由**装机主导**；季节循环形态与峰值随增暖改变；小提琴图反映各 SSP 下场站发电量分布的规模与结构差异。
